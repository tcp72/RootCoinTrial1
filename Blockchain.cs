...
namespace RootCoinTrial1
{
    class Blockchain
    {
        ...
        public Blockchain(int difficulty, decimal miningReward) //constructor
        {
        ...
        public Block CreateGenesisBlock() //need genesis block that is special and not link to other blocks
        {
            return new Block(0, DateTime.Now.ToString("yyyyMMddHHmmssffff"), new List<Transaction>());
        }

        public Block GetLatestBlock() //method to get most recent
        {
            return this.Chain.Last();
        }
        public void AddBlock(Block newBlock)
        {   //get hash of latest, add it to the previous hash property, make new hash, add both together
            newBlock.PreviousHash = this.GetLatestBlock().Hash; //whatever was there become previous hash property of the one about to add
        ...
        public void addPendingTransaction (Transaction transaction)
        {
            ...
            {
                throw new Exception("Transactions must incude a to and from address.");
            }
            ...
            foreach (Block block in this.Chain)
            {
                foreach(Transaction transaction in block.Transactions)
                {
                    ...
                    { 
                        ...
                        {
                            balance -= transaction.Amount; //when pay someone, comes out of your wallet
                        }
                    }
